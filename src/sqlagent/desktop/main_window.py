...
class MainWindow(QMainWindow, PanelAMixin, PanelBMixin, PanelCMixin, RedisPanelsMixin):
    ...
    def _setup_ui(self):
        ...
        central.setStyleSheet(f'QWidget#central{{background:{BG};border-radius:10px;border:1px solid #8B3A3A;}}')
        ...
        rl.setSpacing(8)
        ri = QLabel('🔴'); ri.setAlignment(Qt.AlignCenter); ri.setStyleSheet('font-size: 64px; border:none;')
        rl.addWidget(ri)
        rt = QLabel('Redis'); rt.setAlignment(Qt.AlignCenter)
        rt.setStyleSheet('font-size: 22px; font-weight: bold; color: #00BFA5; border:none;')
        rl.addWidget(rt)
        rd = QLabel('主力 · 键值数据库\nAI驱动 · 全类型编辑 · 智能管理')
        ...
        ml.setSpacing(8)
        mi = QLabel('🐬'); mi.setAlignment(Qt.AlignCenter); mi.setStyleSheet('font-size: 48px; border:none;')
        ml.addWidget(mi)
        mt = QLabel('MySQL'); mt.setAlignment(Qt.AlignCenter)
        mt.setStyleSheet('font-size: 18px; font-weight: bold; color: #A9B7C6; border:none;')
        ml.addWidget(mt)
        md = QLabel('附属 · 关系型数据库\n表结构 · SQL查询 · 数据编辑')
        md.setAlignment(Qt.AlignCenter)
